...
`;
        ...
        let sections: Map<string, Map<string, string>> = new Map<string, Map<string, string>>();

        let current_section: string = "ROOT";
        sections.set("ROOT", new Map<string, string>());

        projectFile.replaceAll("\r", "").split("\n").forEach((line) => {
        ...
        expect(serialized.replace(/\s/g, '')).toMatch(projectFile.replace(/\s/g, ''));
