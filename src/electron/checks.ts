...
import { PROJECTS_FILENAME, INSTALLED_RELEASES_FILENAME } from './constants.js';
...
export async function checkAndUpdateReleases(): Promise<InstalledRelease[]> {
    ...
    return await saveStoredInstalledReleases(releasesFile, releases.filter(r => r.valid));
}
...
export async function checkProjectValid(project: ProjectDetails): Promise<ProjectDetails> {
    ...
    if (!fs.existsSync(project.release.editor_path)) {
        ...
        if (!fs.existsSync(project.launch_path)) {
            ...
            await SetProjectEditorRelease(path.dirname(project.launch_path), project.release);
        }
